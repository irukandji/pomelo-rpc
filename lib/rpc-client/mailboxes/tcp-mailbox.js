...
var MailBox = function (server, opts) {
    ...
    if (this.opts && this.opts.context && this.opts.context.components.__app_monitor__) {
        this.appMonitor = this.opts.context.components.__app_monitor__;
    }
};
...
pro.send = function (tracer, msg, opts, cb) {
    ...
    if (this.bufferMsg) {
        ...
        const msgBuf = this.composer.compose(MSG_TYPE, JSON.stringify(pkg));
        this.socket.write(msgBuf);
        if (this.appMonitor) {
            this.appMonitor.pkgRecord(0, 1, msgBuf.length);
        }
    }
};
...
pro.heartbeat = function () {
    ...
    function ping() {
        ...
        const msgBuf = self.composer.compose(PING);
        self.socket.write(msgBuf);
        self.timer['pong'] = setTimeout(pong, self.pong);
        if (self.appMonitor) {
            self.appMonitor.pkgRecord(0, 1, msgBuf.length);
        }
    }
};
...
var flush = function (mailbox) {
    ...
    const msgBuf = mailbox.composer.compose(MSG_TYPE, JSON.stringify(mailbox.queue));
    mailbox.socket.write(msgBuf);
    mailbox.queue = [];
    if (mailbox.appMonitor) {
        mailbox.appMonitor.pkgRecord(0, 1, msgBuf.length);
    }
};
